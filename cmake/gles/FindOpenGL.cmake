...
Components for GLES targets have been added: ``GLES``, ``GLES2``  and ``GLES3``.
...
.. versionadded:: 3.23
  Additionally, the following OpenGL for Embedded Systems library targets are defined:

``OpenGL::GLES``
  Defined if the system has GLES version 1.
``OpenGL::GLES2``
  Defined if the system has GLES version 2.
``OpenGL::GLES3``
  Defined if the system has GLES version 3.
...
.. versionadded:: 3.23
  New variables for GLES-specific libraries ``GLES``, ``GLES2`` and ``GLES3`` were added:

``OpenGL_GLES_FOUND``
 True, if the system has GLES version 1.
``OPENGL_GLES_INCLUDE_DIR``
 Path to the GLES version 1 include directory.
``OpenGL_GLES2_FOUND``
 True, if the system has GLES version 2..
``OPENGL_GLES2_INCLUDE_DIR``
 Path to the GLES version 2 include directory.
``OpenGL_GLES3_FOUND``
 True, if the system has GLES version 3..
``OPENGL_GLES3_INCLUDE_DIR``
 Path to the GLES version 3 include directory.
...
``OPENGL_gles_LIBRARY``
 Path to the GLES version 1 library.
``OPENGL_gles2_LIBRARY``
 Path to the GLES version 2 library.
``OPENGL_gles3_LIBRARY``
 Path to the GLES version 3 library. Will point to libGLESv2 if no libGLESv3
 is present, but GLES3 headers were found.
...
elseif (APPLE)
  ...
  find_path(OPENGL_GLES_INCLUDE_DIR GLES/gl.h ${_OPENGL_INCLUDE_PATH})
  ...
  find_library(OPENGL_gles_LIBRARY
          NAMES GLESv1_CM GLESv1
          PATHS ${_OPENGL_LIB_PATH}
          )
...
if(OPENGL_gles_LIBRARY AND OPENGL_GLES_INCLUDE_DIR)
  set(OpenGL_GLES_FOUND TRUE)
else()
  set(OpenGL_GLES_FOUND FALSE)
endif()
...
if((OPENGL_gles3_LIBRARY OR OPENGL_gles2_LIBRARY) AND OPENGL_GLES3_INCLUDE_DIR)
...
if(OPENGL_FOUND)
  ...
  # GLES version 1
  if(OpenGL_GLES_FOUND AND NOT TARGET OpenGL::GLES)
    if(IS_ABSOLUTE "${OPENGL_gles_LIBRARY}")
      add_library(OpenGL::GLES UNKNOWN IMPORTED)
      set_target_properties(OpenGL::GLES PROPERTIES
              IMPORTED_LOCATION "${OPENGL_gles_LIBRARY}")
    else()
      add_library(OpenGL::GLES INTERFACE IMPORTED)
      set_target_properties(OpenGL::GLES PROPERTIES
              IMPORTED_LIBNAME "${OPENGL_gles_LIBRARY}")
    endif()
    set_target_properties(OpenGL::GLES PROPERTIES
            INTERFACE_INCLUDE_DIRECTORIES "${OPENGL_INCLUDE_DIR}")
  endif()
