...
public:
    ...
    static PerFrameEqn*
    parse_implicit_per_frame_eqn(std::istream& fs, char* param_string, int index, MilkdropPreset* preset);

    static InitCond*
    parse_per_frame_init_eqn(std::istream& fs, MilkdropPreset* preset, std::map<std::string, Param*>* database);
    ...
    static void readStringUntil(std::istream& fs, std::string* out_buffer, bool wrapAround = true,
                                const std::set<char>& skipList = std::set<char>());
